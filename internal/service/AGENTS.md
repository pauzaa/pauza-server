...
## OVERVIEW
`internal/service` owns domain behavior and transaction boundaries. It orchestrates repositories and support clients for auth, admin, social, sync, RevenueCat webhook reconciliation, and AI-powered analysis.
...
| AI analysis | `internal/service/ai.go` + `ai_prompts.go` | premium-gated prompt composition, AI provider dispatch |
...
- Do not log secrets, OTP values, raw refresh tokens, webhook signatures, or AI API keys.
- Do not embed provider-specific AI logic in the service; delegate to `internal/ai`.
