...
type fakeSyncRepo struct {
	syncModesFn                      func(ctx context.Context, db repository.DBTX, userID string, in syncmodel.TableSync[syncmodel.Mode, string]) (syncmodel.TableResult[syncmodel.Mode, string], error)
	...
	syncRestrictionSessionsFn        func(ctx context.Context, db repository.DBTX, userID string, in syncmodel.TableSync[syncmodel.RestrictionSession, string]) (syncmodel.TableResult[syncmodel.RestrictionSession, string], error)
	...
	recomputeStreakAggregatesFn       func(ctx context.Context, db repository.DBTX, userID string) error
}
...
func (f *fakeSyncRepo) SyncModes(ctx context.Context, db repository.DBTX, userID string, in syncmodel.TableSync[syncmodel.Mode, string]) (syncmodel.TableResult[syncmodel.Mode, string], error) {
...
func (f *fakeSyncRepo) SyncRestrictionSessions(ctx context.Context, db repository.DBTX, userID string, in syncmodel.TableSync[syncmodel.RestrictionSession, string]) (syncmodel.TableResult[syncmodel.RestrictionSession, string], error) {
...
func (f *fakeSyncRepo) RecomputeStreakAggregates(ctx context.Context, db repository.DBTX, userID string) error {
	if f.recomputeStreakAggregatesFn != nil {
		return f.recomputeStreakAggregatesFn(ctx, db, userID)
	}
	return nil
}
...
func TestSync_MissingUser_ReturnsUnauthorized(t *testing.T) {
	...
	repo := &fakeSyncRepo{
		syncModesFn: func(context.Context, repository.DBTX, string, syncmodel.TableSync[syncmodel.Mode, string]) (syncmodel.TableResult[syncmodel.Mode, string], error) {
			t.Fatal("SyncModes should not be called when authenticated user is missing")
			return syncmodel.TableResult[syncmodel.Mode, string]{}, nil
		},
	}
	svc := NewSyncService(
		&fakePool{},
